...
previewCanvas.addEventListener("mouseleave", () => { clearOverlays(); painting = false });

mapCanvas.addEventListener("mousemove", (e) => handleHighlight(e));
mapCanvas.addEventListener("mouseleave", () => { clearOverlays(); painting = false });

refCanvas.addEventListener("mousemove", (e) => handleHighlight(e));
refCanvas.addEventListener("mouseleave", () => { clearOverlays(); painting = false });
...
mapFile.addEventListener('change', (e) => handleDataUpload(e, setMapData), false)
refFile.addEventListener('change', (e) => handleDataUpload(e, setRefData), false)
bakFile.addEventListener('change', (e) => handleDataUpload(e, setBakData), false)
...
async function exportDispMap() {
    ...
    if (window.showSaveFilePicker) {
        ...
        const a = Object.assign(document.createElement('a'), { href: url, download: 'displacement.png' });
        a.click();
...
function displaceImageData(inputImageData, mapImageData) {
    if (inputImageData.width != mapImageData.width || inputImageData.height != mapImageData.height) {
...
function showSizeWarning() {
    ...
    if (mapData) {
        document.getElementById('mapWarn').textContent = `Displacement width:${mapData.width}, height:${mapData.height}`
    }
    else {
        document.getElementById('mapWarn').textContent = ""
    }
    if (refData) {
        document.getElementById('refWarn').textContent = `Reference width:${refData.width}, height:${refData.height}`
    }
    else {
        document.getElementById('refWarn').textContent = ""
    }
    if (bakData) {
        document.getElementById('bakWarn').textContent = `Background width:${bakData.width}, height:${bakData.height}`
    }
    else {
...
function resizeCanvases() {
    ...
    previewStack.style.aspectRatio = Math.min(pixelCanvasWidth / pixelCanvasHeight, backgroundCanvasWidth / backgroundCanvasHeight);
...
function drawControlsPreview(mouseX, mouseY) {
    ...
    if (editMode == 1) {
        if (editStep == 1) {
            drawPixelHighlight(previewOverlayCanvas, toXCoord, toYCoord, "lightblue", "darkblue");
            drawArrowOnCanvas(previewOverlayCanvas, mouseX, mouseY, (toXCoord + 0.5) * pixelSizeX, (toYCoord + 0.5) * pixelSizeY);
        }
    }

    if (editMode == 0) {
        if (editStep == 1) {
            drawArrowOnCanvas(previewOverlayCanvas, (fromXCoord + 0.5) * pixelSizeX, (fromYCoord + 0.5) * pixelSizeY, mouseX, mouseY);
        }
    }
    ...
    if (editMode == 2) {
        ...
        if (deltaX != 0 || deltaY != 0) {
            ...
            drawArrowOnCanvas(previewOverlayCanvas, (pixelMouseX + deltaX + 0.5) * pixelSizeX, (pixelMouseY + deltaY + 0.5) * pixelSizeY, (pixelMouseX + 0.5) * pixelSizeX, (pixelMouseY + 0.5) * pixelSizeY);
            drawPixelHighlight(previewOverlayCanvas, pixelMouseX + deltaX, pixelMouseY + deltaY, "lightblue", "darkblue");
        }
    }
}
...
function drawControlsReference(mouseX, mouseY) {
    ...
    if (editMode == 0) {
        if (editStep == 1) {
            drawPixelHighlight(refOverlayCanvas, fromXCoord, fromYCoord, "lightblue", "darkblue");
            drawArrowOnCanvas(refOverlayCanvas, (fromXCoord + 0.5) * pixelSizeX, (fromYCoord + 0.5) * pixelSizeY, mouseX, mouseY);
        }
    }

    if (editMode == 1) {
        if (editStep == 1) {
            drawArrowOnCanvas(refOverlayCanvas, mouseX, mouseY, (toXCoord + 0.5) * pixelSizeX, (toYCoord + 0.5) * pixelSizeY);
        }
    }
    ...
    if (editMode == 2) {
        ...
        if (deltaX != 0 || deltaY != 0) {
            ...
            drawArrowOnCanvas(refOverlayCanvas, (pixelMouseX + deltaX + 0.5) * pixelSizeX, (pixelMouseY + deltaY + 0.5) * pixelSizeY, (pixelMouseX + 0.5) * pixelSizeX, (pixelMouseY + 0.5) * pixelSizeY);
            drawPixelHighlight(refOverlayCanvas, pixelMouseX + deltaX, pixelMouseY + deltaY, "lightblue", "darkblue");
        }
    }
}
...
function drawPixelHighlight(canvas, pixelX, pixelY, color1 = "black", color2 = "white") {
    drawSelectBoxOnCanvas(canvas, pixelX * pixelSizeX, pixelY * pixelSizeY, pixelSizeX, pixelSizeY, 1, color1, color2);
}

function drawSelectBoxOnCanvas(canvas, x, y, w, h, linewidth, color1 = "black", color2 = "white") {
    var ctx = canvas.getContext("2d");
    ctx.save();
...
function drawArrowOnCanvas(canvas, originX, originY, endX, endY) {
    ...
    ctx.lineTo(endX - unitX * (arrowSize + arrowMargin), endY - unitY * (arrowSize + arrowMargin));
    ctx.lineTo(endX - unitX * (arrowSize + arrowMargin) - unitY * arrowSize, endY - unitY * (arrowSize + arrowMargin) + unitX * arrowSize);
